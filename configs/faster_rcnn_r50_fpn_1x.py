...
model = dict(
    ...
    pretrained='torchvision://resnet50', # 使用的预训练的模型
    # 这里设置网络的backbone
    backbone=dict(
        type='ResNet', # Resnet50 backbone的类型
        depth=50,  #Resnet50的网络层数
        num_stages=4,  # Resnet50中有4个stage.每个stage包含了多个残差block 每个block的卷积层中第三维有着不同的维数
